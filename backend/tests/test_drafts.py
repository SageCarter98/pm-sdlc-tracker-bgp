...
def test_create_resume_and_update_a_draft(client):
    ...
    created = client.put(
        f"/orgs/{tenant_id}/drafts/evidence:abc", json={"base_revision": 0, "form_data": {"note": "first pass"}}
    )
    ...
    updated = client.put(
        f"/orgs/{tenant_id}/drafts/evidence:abc", json={"base_revision": 1, "form_data": {"note": "second pass"}}
    )
...
def test_stale_base_revision_conflicts(client):
    ...
    conflict = client.put(
        f"/orgs/{tenant_id}/drafts/evidence:abc", json={"base_revision": 0, "form_data": {"note": "clobber"}}
    )
...
def test_drafts_are_owner_scoped_not_visible_to_other_tenant_members(client):
    ...
    tenant_id = _create_org_as_admin(client)
    client.put(
        f"/orgs/{tenant_id}/drafts/evidence:abc",
        json={"base_revision": 0, "form_data": {"note": "admin's private draft"}},
    )
